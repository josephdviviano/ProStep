		...
		try:
			if(ppq % self.cur_ppq == 0):			
				...
				if(self.cur_repeats < 0):
					...
					self.cur_step = self.cur_step % self.num_steps					
...
class Tracker:
	...
	def Chain(self,name,repeats):
		self.pattern_chain.append([repeats,name])
		if(self.cur_pattern is None): self.cur_pattern = self.patterns[name]
	...
	def Update(self,ppq,midi_channel):
		self.midi_channel = midi_channel
		x = self.cur_pattern.Update(ppq,self.midi_channel)
...
class Sequencer:
	...
	def Resize(self,num_voices):
		...
		self.num_steps = self.num_banks*16
		for i in range(MAX_SEQS):
	...
	def Randomize(self):
		self.sequencers[self.cur_seq].Randomize()			
		
	def Copy(self):
		self.copy_seq = Voice()
		self.copy_seq.Copy(self.sequencers[self.cur_seq])
	...
	def RandomizeRepeats(self):
		self.sequencers[self.cur_seq].RandomizeRepeats()
	...
	def Update(self,ppq):
		...
		if(self.bTrackerOn == True):
			tracker = self.GetTracker()
			tracker.Update(ppq,self.midi_channel)
		else:
	...
	def AddPatternToTracker(self,name,repeats):
		tracker = self.GetTracker()
		self.Copy()
		self.patterns.append(self.copy_seq)
		tracker.AddPattern(name,self.copy_seq)
		tracker.Chain(name,repeats)
		
	def StartTracker(self):
		self.bTrackerOn = True
		self.bStopped = True
	...
	def HalfNotes(self):
		num_steps = self.sequencers[self.cur_seq].num_steps
		for i in range(0,num_steps):
			self.sequencers[self.cur_seq].ppq[i] = 48
	
	def QuarterNotes(self):
		num_steps = self.sequencers[self.cur_seq].num_steps
		for i in range(0,num_steps):
		
			self.sequencers[self.cur_seq].ppq[i] = 24
	
	def EigthNotes(self):
		num_steps = self.sequencers[self.cur_seq].num_steps
		for i in range(0,num_steps):
		
			self.sequencers[self.cur_seq].ppq[i] = 12
			
	def SixteenthNotes(self):
		num_steps = self.sequencers[self.cur_seq].num_steps
		for i in range(0,num_steps):
		
			self.sequencers[self.cur_seq].ppq[i] = 6
	
	def SetMidiChannel(self,ch):
		self.sequencers[self.cur_seq].midi_channel = ch
		
	def SetActiveTranspose(self,bTranspose):
		self.bActiveTraanspose = bTranspose
...
class Ui_MainWindow(object):
	...
	def UpdateMIDIInput(self,key):
		seq = self.GetSequencer()						
		seq.SetNote(self.cur_step,key[1])
		self.cur_step = self.cur_step + 1
	...
	def ActiveTranspose(self,value):
		seq = self.GetSequencer()
		seq.SetActiveTranspose(value)
		
	def SetChannel(self,value):
		seq = self.GetSequencer()
		seq.SetMidiChannel(value-1)
	...
	def AddTrackerPattern(self):
		...
		dlg.show()
		dlg.exec_()
		if(qdlg.cancel == False): 
			seq = self.GetSequencer()
			seq.AddPatternToTracker(qdlg.pname,qdlg.repeats)
	...
	def setupUi(self, MainWindow,bMDIChild = False):
		...
		self.label1.setGeometry(QtCore.QRect(400,0,50,25))
		...
		self.label2.setGeometry(QtCore.QRect(350,0,50,25))
		...
		self.label3.setGeometry(QtCore.QRect(220,0,50,25))
		self.label3.setObjectName(_fromUtf8("label3"))
		
		self.midiChannel = QtGui.QSpinBox(self.centralwidget)
		self.midiChannel.setGeometry(QtCore.QRect(220,20,50,25))
		self.midiChannel.setObjectName(_fromUtf8("Channel"))
		self.midiChannel.setValue(16)
		self.midiChannel.valueChanged.connect(self.SetChannel)
		
		self.label4 = QtGui.QLabel(self.centralwidget)
		self.label4.setGeometry(QtCore.QRect(60,0,50,25))
		self.label4.setObjectName(_fromUtf8("label4"))

		self.midiOutputs = QtGui.QComboBox(self.centralwidget)
		self.midiOutputs.setGeometry(QtCore.QRect(60, 20, 100, 22))
		self.midiOutputs.setObjectName(_fromUtf8("cbOutputs"))

		outp= mido.get_output_names()
		self.midiOutputs.addItem('None')
		for i in outp:
			self.midiOutputs.addItem(i)

		self.curBank = QtGui.QSpinBox(self.centralwidget)
		self.curBank.setGeometry(QtCore.QRect(170,20,50,25))
		...
		self.layoutWidget.setGeometry(QtCore.QRect(20, 100, 901, 100))
		...
		self.horizontalLayout_3.setObjectName(_fromUtf8("horizontalLayout_3"))		
		self.horizontalLayout_3.setGeometry(QtCore.QRect(20, 10, 901, 100))
		
		self.repeat1 = QtGui.QDial(self.layoutWidget)
		...
		self.repeat2 = QtGui.QDial(self.layoutWidget)
		...
		self.repeat3 = QtGui.QDial(self.layoutWidget)
		...
		self.repeat4 = QtGui.QDial(self.layoutWidget)
		...
		self.repeat5 = QtGui.QDial(self.layoutWidget)
		...
		self.repeat6 = QtGui.QDial(self.layoutWidget)
		...
		self.repeat7 = QtGui.QDial(self.layoutWidget)
		...
		self.repeat8 = QtGui.QDial(self.layoutWidget)
		...
		self.repeat9 = QtGui.QDial(self.layoutWidget)
		...
		self.repeat10 = QtGui.QDial(self.layoutWidget)
		...
		self.repeat11 = QtGui.QDial(self.layoutWidget)
		...
		self.repeat12 = QtGui.QDial(self.layoutWidget)
		...
		self.repeat13 = QtGui.QDial(self.layoutWidget)
		...
		self.repeat14 = QtGui.QDial(self.layoutWidget)
		...
		self.repeat15 = QtGui.QDial(self.layoutWidget)
		...
		self.repeat16 = QtGui.QDial(self.layoutWidget)
		...
		self.widget.setGeometry(QtCore.QRect(20, 50, 901, 100))
		self.widget.setObjectName(_fromUtf8("widget"))
		self.horizontalLayout = QtGui.QHBoxLayout(self.widget)
		self.horizontalLayout.setObjectName(_fromUtf8("horizontalLayout"))
		self.note1 = QtGui.QDial(self.widget)
		...
		self.note2 = QtGui.QDial(self.widget)
		...
		self.note3 = QtGui.QDial(self.widget)
		...
		self.note4 = QtGui.QDial(self.widget)
		...
		self.note5 = QtGui.QDial(self.widget)
		...
		self.note6 = QtGui.QDial(self.widget)
		...
		self.note7 = QtGui.QDial(self.widget)
		...
		self.note8 = QtGui.QDial(self.widget)
		...
		self.note9 = QtGui.QDial(self.widget)
		...
		self.note10 = QtGui.QDial(self.widget)
		...
		self.note11 = QtGui.QDial(self.widget)
		...
		self.note12 = QtGui.QDial(self.widget)
		...
		self.note13 = QtGui.QDial(self.widget)
		...
		self.note14 = QtGui.QDial(self.widget)
		...
		self.note15 = QtGui.QDial(self.widget)
		...
		self.note16 = QtGui.QDial(self.widget)
		...
		self.widget1.setGeometry(QtCore.QRect(30, 170, 901, 45))
		...
		self.widget2.setGeometry(QtCore.QRect(20, 200, 901, 45))
		...
		self.actionActiveTranspose = QtGui.QAction(MainWindow,checkable=True)
		self.actionActiveTranspose.setObjectName(_fromUtf8("actionActiveTranspose"))
		self.actionActiveTranspose.triggered.connect(self.ActiveTranspose)		
		
		self.actionHalfNotes = QtGui.QAction(MainWindow)
		self.actionHalfNotes.setObjectName(_fromUtf8("actionHalfNotes"))
		self.actionHalfNotes.triggered.connect(self.HalfNotes)
		self.actionQuarterNotes = QtGui.QAction(MainWindow)
		self.actionQuarterNotes.setObjectName(_fromUtf8("actionQuarterNotes"))
		self.actionQuarterNotes.triggered.connect(self.QuarterNotes)
		self.actionEigthNotes = QtGui.QAction(MainWindow)
		self.actionEigthNotes.setObjectName(_fromUtf8("actionEigthNotes"))
		self.actionEigthNotes.triggered.connect(self.EigthNotes)
		self.actionSixteenthNotes = QtGui.QAction(MainWindow)
		self.actionSixteenthNotes.setObjectName(_fromUtf8("actionSixteenthNotes"))
		self.actionSixteenthNotes.triggered.connect(self.SixteenthNotes)
		...
		self.actionAddTrackerPattern = QtGui.QAction(MainWindow)
		self.actionAddTrackerPattern.setObjectName(_fromUtf8("actionDelPattern"))
		self.actionAddTrackerPattern.triggered.connect(self.AddTrackerPattern)
		
		self.actionStartTracker = QtGui.QAction(MainWindow)
		self.actionStartTracker.setObjectName(_fromUtf8("actionDelPattern"))
		self.actionStartTracker.triggered.connect(self.StartTracker)
		
		self.actionStopTracker = QtGui.QAction(MainWindow)
		self.actionStopTracker.setObjectName(_fromUtf8("actionDelPattern"))
		self.actionStopTracker.triggered.connect(self.StopTracker)
				
		
		self.actionSync = QtGui.QAction(MainWindow)
		self.actionSync.setObjectName(_fromUtf8("actionSyncPorts"))
		...
		self.menuScene.addAction(self.actionActiveTranspose)
		self.menuScene.addAction(self.actionHalfNotes)
		self.menuScene.addAction(self.actionQuarterNotes)
		self.menuScene.addAction(self.actionEigthNotes)
		self.menuScene.addAction(self.actionSixteenthNotes)
		...
		self.menuTracker.addAction(self.actionAddTrackerPattern)
		self.menuTracker.addAction(self.actionStartTracker)
		self.menuTracker.addAction(self.actionStopTracker)
	...
	def retranslateUi(self, MainWindow):
		...
		self.label1.setText(_translate("MainWindow","Steps",None))
		self.label2.setText(_translate("MainWindow","Row",None))
		self.label3.setText(_translate("MainWindow","Voice",None))
		self.label3.setText(_translate("MainWindow","Channel",None))
		...
		self.menuTracker.setTitle(_translate("MainWindow","Tracker",None))
		...
		self.actionHalfNotes.setText(_translate("MainWindow", "Half", None))
		self.actionQuarterNotes.setText(_translate("MainWindow", "Quarter", None))
		self.actionEigthNotes.setText(_translate("MainWindow", "Eigth", None))
		self.actionSixteenthNotes.setText(_translate("MainWindow", "Sixteenth", None))
		...
		self.actionSync.setText(_translate("MainWindow", "Sync Setup", None))
				
		self.actionAddPattern.setText(_translate("MainWindow","Add Pattern",None))
		self.actionDelPattern.setText(_translate("MainWindow","Delete Pattern",None))
		...
		self.actionAddTrackerPattern.setText(_translate("MainWindow","Add Pattern",None))
		self.actionStartTracker.setText(_translate("MainWindow","Start",None))
		self.actionStopTracker.setText(_translate("MainWindow","Stop",None))
		self.actionActiveTranspose.setText(_translate("MainWindow","Active Transpose",None))
def run(app):
	sys.exit(app.exec_())
	MIDI_Shutdown()
...
if __name__ == '__main__':
	...
	app.setStyleSheet("""
		...
		QHorizontalSlider { border:20px solid black;
			    border-radius: 10px;
