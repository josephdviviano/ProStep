...
def MIDI_ProcessCC(msg):
	...
	if(cc == 114 and msg[2] == 127):
		...
		cur_seq = cur_seq+1
		cur_seq = cur_seq % 4
		if(cur_seq == 0): seq_select = NRPN_SEQ1_STEP1
		elif(cur_seq == 1): seq_select = NRPN_SEQ2_STEP1
		elif(cur_seq == 2): seq_select = NRPN_SEQ3_STEP1
		else: seq_select = NRPN_SEQ4_STEP1
		...
		MOPHO_SendSeqStep(seq_select,0+seq_assign,val,chan)
	elif(cc == 15 ):
		MOPHO_SendSeqStep(seq_select,1+seq_assign,val,chan)
	elif(cc == 16 ):
		MOPHO_SendSeqStep(seq_select,2+seq_assign,val,chan)
	elif(cc == 17 ):
		MOPHO_SendSeqStep(seq_select,3+seq_assign,val,chan)
	elif(cc == 18 ):
		MOPHO_SendSeqStep(seq_select,4+seq_assign,val,chan)
	elif(cc == 19 ):
		MOPHO_SendSeqStep(seq_select,5+seq_assign,val,chan)
	elif(cc == 20 ):
		MOPHO_SendSeqStep(seq_select,6+seq_assign,val,chan)
	elif(cc == 21 ):
		MOPHO_SendSeqStep(seq_select,7+seq_assign,val,chan)
	...
	while 1:
		...
		for msg in midi_input:
			...
			if(key[0] == 0x90+TETRA_CHANNEL):
				...
				MOPHO_SendSeqStep(NRPN_SEQ1_STEP1,cur_step,note,TETRA_CHANNEL)
				cur_step = cur_step+1
				cur_step = cur_step % 16
			
				
			elif(key[0] >= 0xB0 and key[0] <= 0xBF):
				if((key[0] & 0xF) == TETRA_CHANNEL and key[1] == 115): 
					TETRA_ChordOn(root)
				else:
					MIDI_ProcessCC(key)
