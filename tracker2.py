...
class Track:
	...
	def Tick(self):
		self.track[self.cur_step].Tick()
		self.ppq = self.ppq + 1
		
		if(self.ppq >= PPQ_WHOLE):
			if( self.cur_measure in self.midi_msg):
				print self.midi_msg[self.cur_measure]
				for msg in self.midi_msg[self.cur_measure]:
					midi_output.send(MSG(msg))
			self.ppq = 0
			self.cur_step = self.cur_step + 1
			self.cur_step = self.cur_step % len(self.track)
			self.cur_measure = self.cur_measure + 1
			self.cur_measure = self.cur_measure % 4
...
mopho.AddPattern(pattern1)
mopho.AddPattern(pattern1)

pattern[0] = [ notes['D#3'],notes['D#3'],notes['D#2'],notes['D#3'] ]
pattern[1] = [ notes['D#3'],notes['D#3'],notes['D#2'],notes['D#3'] ]
pattern[2] = [ notes['D#3'],notes['D#3'],notes['D#2'],notes['D#3'] ]
pattern[3] = [ notes['D#3'],notes['D#3'],notes['D#2'],notes['D#4'] ]
